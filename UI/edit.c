...
static void push_glyph_quad(edit_t* edit, const glyph_t* g, const f32 pen_x, const f32 pen_y) {
    ...
    if (!Mesh(push)(edit->mesh, (mesh_buf_t){
        .count = 6,
        .data = quad,
        .is_vertices = true
    })) {
        logError(ERR_COMPONENT_SYSTEM, "Failed to resize edit text mesh.");
    }
}
static void build_text_mesh(edit_t* edit, const f32 start_x, const f32 start_y) {
    ...
    Mesh(bind)(get_root(edit), edit->mesh);
    Font(bind)(edit->font);
...
static void private(set_caret_position)(edit_t* edit, const f64 mouse_x, const f64 mouse_y) {
    ...
    if (edit->mesh->vertices.count >= QUAD_SIZE) edit->mesh->vertices.count -= QUAD_SIZE;
    ...
    Mesh(bind)(get_root(edit), edit->mesh);
    Font(bind)(edit->font);
    pen.x -= (f32)caret_glyph->offset.x;
    const f32 x0 = pen.x + caret_glyph->offset.x;
    const f32 y0 = pen.y + caret_glyph->offset.y;
    const f32 x1 = x0 + caret_glyph->dim.width;
    const f32 y1 = y0 + caret_glyph->dim.height;

    const f32 quad[24] = {
        x0, y1, caret_glyph->x0, caret_glyph->y1,
        x1, y0, caret_glyph->x1, caret_glyph->y0,
        x0, y0, caret_glyph->x0, caret_glyph->y0,
        x0, y1, caret_glyph->x0, caret_glyph->y1,
        x1, y1, caret_glyph->x1, caret_glyph->y1,
        x1, y0, caret_glyph->x1, caret_glyph->y0
    };
    Mesh(write)(edit->mesh, (mesh_buf_t){
        .offset = edit->mesh->vertices.count,
        .count = 6,
        .data = quad,
        .is_vertices = true
    });
...
edit_t* Edit(new)(void* parent, const style_group_t* group, const bounding_box* box) {
    ...
    edit->mesh = Mesh(new)(frame, (mesh_param_t){
        .tag = DYNAMIC_MESH,
        .capacity = QUAD_SIZE * DEFAULT_CAPACITY,
        .attributes = MESH_2D | MESH_UV0
    });
    if (!edit->mesh) {
        logError(ERR_MESH_CACHE, "Failed to create text mesh for edit.");
        goto cleanup;
    }
...
void Edit(update)(edit_t* edit) {
    ...
    Mesh(sub_draw)(edit->mesh, QUAD_SIZE * (frame->focused.instance == edit ? edit->mesh->vertices.count : edit->mesh->vertices.count - QUAD_SIZE), 0);
